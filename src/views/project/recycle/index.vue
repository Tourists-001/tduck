    ...
    >
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="text" @click="restoreProject(scope.row.key, 1)">
            恢复
          </el-button>
          <el-popconfirm
            title="确定删除该项目吗？"
...
<script>
import {
  listRecycleFormRequest,
  restoreRecycleFormRequest,
  deleteRecycleFormRequest
...
export default {
  ...
  methods: {
    // 获取数据
    async queryRecycleProjectPage() {
      const res = await listRecycleFormRequest(this.queryParams)
    ...
    // 恢复表单
    async restoreProject(key) {
      const { data } = await restoreRecycleFormRequest({ key })
      if (data) {
    ...
    async deleteProject(key) {
      const { data } = await deleteRecycleFormRequest({ key })
      console.log(data)
      if (data) {
