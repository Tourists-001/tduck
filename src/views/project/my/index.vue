      ...
      <el-form ref="form" label-width="100px">
        <el-form-item label="项目更新时间">
          ...
          <el-input
            ...
            type="text"
          />
          <el-button class="ml-20" type="primary" @click="queryProjectPage">
            查询
          </el-button>
        </el-form-item>
...
export default {
  ...
  computed: {
    getStatusColorStyle() {
      return function(code) {
        const color = this.projectStatusList.find(
          (item) => item.code === code
        ).color
        return {
  ...
  methods: {
    ...
    // 获取表单数据
    async queryProjectPage() {
      const res = await pageFormRequest(this.queryParams)
    ...
    toProjectHandle(key, type) {
      this.$router.push({
        ...
        query: { key: key, active: type }
      })
    },
    // 停止收集
    async stopProject(key) {
      const res = await stopFormRequest({ key })
      // console.log(res);
      if (res.data) {
    ...
    async deleteProject(key) {
      const res = await deleteFormRequest({ key })
      if (res.data) {
